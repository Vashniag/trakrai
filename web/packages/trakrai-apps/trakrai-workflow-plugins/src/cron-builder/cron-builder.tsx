'use client';

import React, { useEffect, useState, useMemo, useCallback, useRef } from 'react';

import { Input } from '@trakrai/design-system/components/input';
import { ToggleGroup, ToggleGroupItem } from '@trakrai/design-system/components/toggle-group';
import cronstrue from 'cronstrue';

interface CronBuilderProps {
  onChange: (cronExpression: string) => void;
  defaultValue?: string;
  className?: string;
}

interface ScheduleType {
  value: string;
  label: string;
}

interface ParsedCron {
  type: string;
  values: {
    minutes?: number[];
    hours?: number[];
    daysOfMonth?: number[];
    months?: number[];
    daysOfWeek?: number[];
    custom?: string;
  };
}

interface CronTextResult {
  status: boolean;
  value?: string;
}

const getCronText = (cronString: string): CronTextResult => {
  try {
    const value = cronstrue.toString(cronString.trim());
    return { status: true, value };
  } catch {
    return { status: false };
  }
};

const SCHEDULE_TYPES: ScheduleType[] = [
  { value: 'never', label: 'Never' },
  { value: 'hour', label: 'Hourly' },
  { value: 'day', label: 'Daily' },
  { value: 'week', label: 'Weekly' },
  { value: 'month', label: 'Monthly' },
  { value: 'year', label: 'Yearly' },
  { value: 'custom', label: 'Custom' },
];

const MINUTES = Array.from({ length: 60 }, (_, i) => i);
const HOURS = Array.from({ length: 24 }, (_, i) => i);
const DAYS_OF_MONTH = Array.from({ length: 31 }, (_, i) => i + 1);
const MINUTE_INTERVAL = 5;
const COMMON_MINUTES = Array.from({ length: 12 }, (_, i) => i * MINUTE_INTERVAL);
const MONTHS_SHORT = [
  'Jan',
  'Feb',
  'Mar',
  'Apr',
  'May',
  'Jun',
  'Jul',
  'Aug',
  'Sep',
  'Oct',
  'Nov',
  'Dec',
];
const DAYS_SHORT = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

const getButtonStyles = (state = 'default'): string => {
  const base = 'px-2 py-1 text-xs rounded border transition-colors';

  switch (state) {
    case 'selected':
      return `${base} bg-[hsl(var(--selected))] text-[hsl(var(--selected-foreground))] border-[hsl(var(--selected))]`;
    case 'pressed':
      return `${base} bg-[hsl(var(--pressed))] text-[hsl(var(--pressed-foreground))] border-[hsl(var(--pressed))]`;
    case 'disabled':
      return `${base} bg-[hsl(var(--disabled))] text-[hsl(var(--disabled-foreground))] border-[hsl(var(--disabled))] cursor-not-allowed`;
    case 'loading':
      return `${base} bg-[hsl(var(--processing))] text-[hsl(var(--processing-foreground))] border-[hsl(var(--processing))]`;
    case 'focused':
      return `${base} bg-[hsl(var(--focused))] text-[hsl(var(--focused-foreground))] border-[hsl(var(--focused))] outline-none ring-2 ring-[hsl(var(--focused)/50%)]`;
    default:
      return `${base} bg-background text-foreground border-border hover:bg-[hsl(var(--selected))] hover:text-[hsl(var(--selected-foreground))] focus:bg-[hsl(var(--focused))] focus:text-[hsl(var(--focused-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--focused)/50%)] active:bg-[hsl(var(--pressed))] active:text-[hsl(var(--pressed-foreground))]`;
  }
};

interface GridButtonProps {
  value: number | string;
  isSelected: boolean;
  onClick: (value: number | string) => void;
  children?: React.ReactNode;
  minWidth?: string;
  className?: string;
  disabled?: boolean;
}

const GridButton = React.memo<GridButtonProps>(
  ({
    value,
    isSelected,
    onClick,
    children,
    minWidth = '36px',
    className = '',
    disabled = false,
  }: GridButtonProps) => {
    const [isPressed, setIsPressed] = useState(false);

    const getButtonState = () => {
      if (disabled) {
        return 'disabled';
      }
      if (isPressed) {
        return 'pressed';
      }
      if (isSelected) {
        return 'selected';
      }
      return 'default';
    };

    return (
      <button
        className={`${getButtonStyles(getButtonState())} ${className}`}
        disabled={disabled}
        style={{ minWidth }}
        type="button"
        onClick={() => {
          if (!disabled) {
            onClick(value);
          }
        }}
        onMouseDown={() => {
          if (!disabled) {
            setIsPressed(true);
          }
        }}
        onMouseLeave={() => {
          setIsPressed(false);
        }}
        onMouseUp={() => {
          setIsPressed(false);
        }}
        onTouchEnd={() => {
          setIsPressed(false);
        }}
        onTouchStart={() => {
          if (!disabled) {
            setIsPressed(true);
          }
        }}
      >
        {children ?? (typeof value === 'number' ? value.toString().padStart(2, '0') : value)}
      </button>
    );
  },
);

GridButton.displayName = 'GridButton';

interface ScheduleFieldsProps {
  scheduleType: string;
  renderDaysOfWeekList: () => React.ReactNode;
  renderMonthsGrid: () => React.ReactNode;
  renderDaysOfMonthGrid: () => React.ReactNode;
  renderHoursGrid: () => React.ReactNode;
  renderMinutesGrid: () => React.ReactNode;
}

const ScheduleFields = React.memo<ScheduleFieldsProps>(
  ({
    scheduleType,
    renderDaysOfWeekList,
    renderMonthsGrid,
    renderDaysOfMonthGrid,
    renderHoursGrid,
    renderMinutesGrid,
  }: ScheduleFieldsProps) => {
    if (scheduleType === 'week') {
      return (
        <div className="flex flex-1 flex-col gap-4">
          <div className="w-full">{renderDaysOfWeekList()}</div>
          <div className="flex flex-wrap items-start gap-6">
            {renderHoursGrid()}
            {renderMinutesGrid()}
          </div>
        </div>
      );
    }

    if (scheduleType === 'year') {
      return (
        <div className="flex flex-1 flex-col gap-4">
          <div className="flex flex-wrap items-start gap-6">
            {renderDaysOfMonthGrid()}
            {renderMonthsGrid()}
          </div>
          <div className="flex flex-wrap items-start gap-6">
            {renderHoursGrid()}
            {renderMinutesGrid()}
          </div>
        </div>
      );
    }

    if (scheduleType === 'month') {
      return (
        <div className="flex flex-1 flex-col gap-4">
          <div className="flex flex-wrap items-start gap-6">{renderDaysOfMonthGrid()}</div>
          <div className="flex flex-wrap items-start gap-6">
            {renderHoursGrid()}
            {renderMinutesGrid()}
          </div>
        </div>
      );
    }

    if (scheduleType === 'day') {
      return (
        <div className="flex flex-1 flex-wrap items-start gap-6">
          {renderHoursGrid()}
          {renderMinutesGrid()}
        </div>
      );
    }

    if (scheduleType === 'hour') {
      return <div className="flex flex-1 flex-wrap items-start gap-6">{renderMinutesGrid()}</div>;
    }

    return null;
  },
);

ScheduleFields.displayName = 'ScheduleFields';

interface MonthButtonProps {
  month: string;
  index: number;
  isSelected: boolean;
  onClick: (index: number) => void;
}

const MonthButton = React.memo<MonthButtonProps>(
  ({ month, index, isSelected, onClick }: MonthButtonProps) => {
    const [isPressed, setIsPressed] = useState(false);

    return (
      <button
        key={index}
        className={`px-3 py-1 ${getButtonStyles(getToggleButtonState(isPressed, isSelected))}`}
        type="button"
        onClick={() => {
          onClick(index);
        }}
        onMouseDown={() => {
          setIsPressed(true);
        }}
        onMouseLeave={() => {
          setIsPressed(false);
        }}
        onMouseUp={() => {
          setIsPressed(false);
        }}
        onTouchEnd={() => {
          setIsPressed(false);
        }}
        onTouchStart={() => {
          setIsPressed(true);
        }}
      >
        {month}
      </button>
    );
  },
);

MonthButton.displayName = 'MonthButton';

interface DayOfWeekButtonProps {
  day: string;
  index: number;
  isSelected: boolean;
  isWeekend: boolean;
  onClick: (index: number) => void;
}

const DayOfWeekButton = React.memo<DayOfWeekButtonProps>(
  ({ day, index, isSelected, isWeekend, onClick }: DayOfWeekButtonProps) => {
    const [isPressed, setIsPressed] = useState(false);

    return (
      <button
        key={index}
        className={`px-3 py-1 text-center ${getButtonStyles(getToggleButtonState(isPressed, isSelected))} ${isWeekend ? 'text-orange-500' : ''}`}
        style={{ minWidth: '50px' }}
        type="button"
        onClick={() => {
          onClick(index);
        }}
        onMouseDown={() => {
          setIsPressed(true);
        }}
        onMouseLeave={() => {
          setIsPressed(false);
        }}
        onMouseUp={() => {
          setIsPressed(false);
        }}
        onTouchEnd={() => {
          setIsPressed(false);
        }}
        onTouchStart={() => {
          setIsPressed(true);
        }}
      >
        {day}
      </button>
    );
  },
);

DayOfWeekButton.displayName = 'DayOfWeekButton';

const getToggleButtonState = (isPressed: boolean, isSelected: boolean): string => {
  if (isPressed) {
    return 'pressed';
  }
  if (isSelected) {
    return 'selected';
  }
  return 'default';
};

const CRON_PARTS_COUNT = 5;
const MONTHS_IN_YEAR = 12;
const DAYS_IN_MONTH = 31;
const DAYS_IN_WEEK = 7;
const HOURS_IN_DAY = 24;
const MINUTES_IN_HOUR = 60;
const WEEKEND_SUNDAY = 0;
const WEEKEND_SATURDAY = 6;

export const CronBuilder = ({ onChange, defaultValue, className }: CronBuilderProps) => {
  const defaultSchedule = defaultValue ?? '0 0 * * 0';
  const onChangeRef = useRef(onChange);

  const parseCronExpression = (cronExpr: string): ParsedCron => {
    if (cronExpr === '') {
      return { type: 'never', values: {} };
    }
    const cleanExpr = cronExpr.trim();
    const parts = cleanExpr.split(' ');
    if (parts.length !== CRON_PARTS_COUNT) {
      return { type: 'custom', values: { custom: cleanExpr } };
    }
    const [min, hour, dom, month, dow] = parts;
    if (
      min === undefined ||
      hour === undefined ||
      dom === undefined ||
      month === undefined ||
      dow === undefined
    ) {
      return { type: 'custom', values: { custom: cleanExpr } };
    }
    const parseNumbers = (part: string): number[] => {
      if (part === '*' || part === '?') {
        return [];
      }
      return part
        .split(',')
        .map((v) => parseInt(v.trim(), 10))
        .filter((v) => !isNaN(v));
    };

    try {
      if (min !== '*' && hour === '*' && dom === '*' && month === '*' && dow === '?') {
        return { type: 'hour', values: { minutes: parseNumbers(min) } };
      }
      if (min !== '*' && hour !== '*' && dom === '*' && month === '*' && dow === '?') {
        return {
          type: 'day',
          values: {
            minutes: parseNumbers(min),
            hours: parseNumbers(hour),
          },
        };
      }
      if (min !== '*' && hour !== '*' && dom === '?' && month === '*' && dow !== '*') {
        return {
          type: 'week',
          values: {
            minutes: parseNumbers(min),
            hours: parseNumbers(hour),
            daysOfWeek: parseNumbers(dow),
          },
        };
      }
      if (min !== '*' && hour !== '*' && dom !== '*' && month === '*' && dow === '?') {
        return {
          type: 'month',
          values: {
            minutes: parseNumbers(min),
            hours: parseNumbers(hour),
            daysOfMonth: parseNumbers(dom),
          },
        };
      }
      return { type: 'custom', values: { custom: cleanExpr } };
    } catch {
      return { type: 'custom', values: { custom: cleanExpr } };
    }
  };

  const initialParsed = parseCronExpression(defaultSchedule);

  const [scheduleType, setScheduleType] = useState(initialParsed.type);
  const [minutes, setMinutes] = useState<number[]>(initialParsed.values.minutes ?? [0]);
  const [hours, setHours] = useState<number[]>(initialParsed.values.hours ?? [0]);
  const [daysOfMonth, setDaysOfMonth] = useState<number[]>(initialParsed.values.daysOfMonth ?? [1]);
  const [months, setMonths] = useState<number[]>(initialParsed.values.months ?? [1]);
  const [daysOfWeek, setDaysOfWeek] = useState<number[]>(initialParsed.values.daysOfWeek ?? [0]);
  const [custom, setCustom] = useState<string>(initialParsed.values.custom ?? defaultSchedule);
  const [showAllMinutes, setShowAllMinutes] = useState(false);

  const loadDefaults = () => {
    setMinutes([0]);
    setHours([0]);
    setDaysOfMonth([1]);
    setMonths([1]);
    setDaysOfWeek([0]);
  };

  const cronExpression = useMemo(() => {
    let expression = '';

    const cleanMonths = months;
    const cleanDaysOfMonth = daysOfMonth;
    const cleanDaysOfWeek = daysOfWeek;
    const cleanHours = hours;
    const cleanMinutes = minutes;

    const monthsCSV = cleanMonths.length === MONTHS_IN_YEAR ? '*' : cleanMonths.join(',');
    const domCSV = cleanDaysOfMonth.length === DAYS_IN_MONTH ? '*' : cleanDaysOfMonth.join(',');
    const dowCSV = cleanDaysOfWeek.length === DAYS_IN_WEEK ? '*' : cleanDaysOfWeek.join(',');
    const hoursCSV = cleanHours.length === HOURS_IN_DAY ? '*' : cleanHours.join(',');
    const minutesCSV = cleanMinutes.length === MINUTES_IN_HOUR ? '*' : cleanMinutes.join(',');

    switch (scheduleType) {
      case 'hour':
        expression = `${minutesCSV} * * * ?`;
        break;
      case 'day':
        expression = `${minutesCSV} ${hoursCSV} * * ?`;
        break;
      case 'week':
        expression = `${minutesCSV} ${hoursCSV} ? * ${dowCSV}`;
        break;
      case 'month':
        expression = `${minutesCSV} ${hoursCSV} ${domCSV} * ?`;
        break;
      case 'year':
        expression = `${minutesCSV} ${hoursCSV} ${domCSV} ${monthsCSV} ?`;
        break;
      case 'custom':
        expression = custom;
        break;
      default:
    }
    return expression;
  }, [scheduleType, minutes, hours, daysOfMonth, months, daysOfWeek, custom]);

  useEffect(() => {
    onChangeRef.current = onChange;
  }, [onChange]);

  useEffect(() => {
    onChangeRef.current(cronExpression);
  }, [cronExpression]);

  const handleMonthToggle = useCallback((monthIndex: number | string) => {
    const monthNum = (typeof monthIndex === 'number' ? monthIndex : parseInt(monthIndex, 10)) + 1;
    setMonths((prev) =>
      prev.includes(monthNum) ? prev.filter((m) => m !== monthNum) : [...prev, monthNum],
    );
  }, []);

  const renderMonthsGrid = () => (
    <div className="flex w-fit flex-col gap-2">
      <span className="text-foreground text-xs font-medium">Months</span>
      <div className="grid w-fit grid-cols-3 gap-1">
        {MONTHS_SHORT.map((month, index) => (
          <MonthButton
            key={month}
            index={index}
            isSelected={months.includes(index + 1)}
            month={month}
            onClick={handleMonthToggle}
          />
        ))}
      </div>
    </div>
  );

  const handleDayOfWeekToggle = useCallback((dayIndex: number | string) => {
    const dayNum = typeof dayIndex === 'number' ? dayIndex : parseInt(dayIndex, 10);
    setDaysOfWeek((prev) =>
      prev.includes(dayNum) ? prev.filter((d) => d !== dayNum) : [...prev, dayNum],
    );
  }, []);

  const renderDaysOfWeekList = () => {
    const weekendDays = [WEEKEND_SUNDAY, WEEKEND_SATURDAY];

    return (
      <div className="flex w-full flex-col gap-2">
        <span className="text-foreground text-xs font-medium">Days of Week</span>
        <div className="flex w-full flex-row justify-start gap-1">
          {DAYS_SHORT.map((day, index) => {
            const isWeekend = weekendDays.includes(index);
            const isSelected = daysOfWeek.includes(index);
            return (
              <DayOfWeekButton
                key={day}
                day={day}
                index={index}
                isSelected={isSelected}
                isWeekend={isWeekend}
                onClick={handleDayOfWeekToggle}
              />
            );
          })}
        </div>
      </div>
    );
  };

  const handleDayOfMonthToggle = useCallback((day: number | string) => {
    const dayNum = typeof day === 'number' ? day : parseInt(day, 10);
    setDaysOfMonth((prev) =>
      prev.includes(dayNum) ? prev.filter((d) => d !== dayNum) : [...prev, dayNum],
    );
  }, []);

  const renderDaysOfMonthGrid = () => (
    <div className="flex w-fit flex-col gap-2">
      <span className="text-foreground text-xs font-medium">Days of Month</span>
      <div className="grid w-fit grid-cols-7 gap-1">
        {DAYS_OF_MONTH.map((day) => (
          <GridButton
            key={day}
            isSelected={daysOfMonth.includes(day)}
            value={day}
            onClick={handleDayOfMonthToggle}
          />
        ))}
      </div>
    </div>
  );

  const handleHourToggle = useCallback((hour: number | string) => {
    const hourNum = typeof hour === 'number' ? hour : parseInt(hour, 10);
    setHours((prev) =>
      prev.includes(hourNum) ? prev.filter((h) => h !== hourNum) : [...prev, hourNum],
    );
  }, []);

  const renderHoursGrid = () => (
    <div className="flex w-fit flex-col gap-2">
      <span className="text-foreground text-xs font-medium">Hours</span>
      <div className="grid w-fit grid-cols-6 gap-1">
        {HOURS.map((hour) => (
          <GridButton
            key={hour}
            isSelected={hours.includes(hour)}
            value={hour}
            onClick={handleHourToggle}
          />
        ))}
      </div>
    </div>
  );

  const handleMinuteToggle = useCallback((minute: number | string) => {
    const minuteNum = typeof minute === 'number' ? minute : parseInt(minute, 10);
    setMinutes((prev) =>
      prev.includes(minuteNum) ? prev.filter((m) => m !== minuteNum) : [...prev, minuteNum],
    );
  }, []);

  const minutesToShow = useMemo(
    () => (showAllMinutes ? MINUTES : COMMON_MINUTES),
    [showAllMinutes],
  );

  const renderMinutesGrid = () => (
    <div className="flex w-fit flex-col gap-2">
      <div className="flex items-center gap-2">
        <span className="text-foreground text-xs font-medium">Minutes</span>
        <span className="text-muted-foreground text-xs">({showAllMinutes ? 'All' : 'Common'})</span>
        <button
          aria-label={showAllMinutes ? 'Show fewer minutes' : 'Show more minutes'}
          className="border-border hover:bg-accent rounded border px-1 pb-0.5 text-xs transition-colors"
          type="button"
          onClick={() => {
            setShowAllMinutes(!showAllMinutes);
          }}
        >
          {showAllMinutes ? '-' : '+'}
        </button>
      </div>
      <div className={`grid w-fit gap-1 ${showAllMinutes ? 'grid-cols-12' : 'grid-cols-6'}`}>
        {minutesToShow.map((minute) => (
          <GridButton
            key={minute}
            isSelected={minutes.includes(minute)}
            value={minute}
            onClick={handleMinuteToggle}
          />
        ))}
      </div>
    </div>
  );

  return (
    <div className={`flex flex-grow flex-col ${className ?? ''}`}>
      <div className="flex h-full flex-col gap-3">
        <div className="flex flex-col gap-2">
          <ToggleGroup
            className="w-fit justify-start"
            type="single"
            value={scheduleType}
            variant="outline"
            onValueChange={(value) => {
              if (value !== '') {
                loadDefaults();
                setScheduleType(value);
              }
            }}
          >
            {SCHEDULE_TYPES.map(({ value, label }) => (
              <ToggleGroupItem key={value} className="h-8 px-3 py-1 text-xs" value={value}>
                {label}
              </ToggleGroupItem>
            ))}
          </ToggleGroup>
        </div>

        {scheduleType === 'never' ? (
          <div className="text-muted-foreground mt-2 text-sm">No schedule configured</div>
        ) : (
          <div className="flex h-full flex-col gap-4">
            {scheduleType === 'custom' ? (
              <div className="bg-card border-input flex h-full w-full flex-1 flex-col gap-2 border p-3">
                <label
                  className="text-card-foreground text-xs font-medium"
                  htmlFor="custom-schedule"
                >
                  Cron Expression
                </label>
                <Input
                  id="custom-schedule"
                  name="schedule"
                  placeholder="0 0 * * 0"
                  type="text"
                  value={custom}
                  onChange={(event) => {
                    setCustom(event.target.value);
                  }}
                />
              </div>
            ) : (
              <ScheduleFields
                renderDaysOfMonthGrid={renderDaysOfMonthGrid}
                renderDaysOfWeekList={renderDaysOfWeekList}
                renderHoursGrid={renderHoursGrid}
                renderMinutesGrid={renderMinutesGrid}
                renderMonthsGrid={renderMonthsGrid}
                scheduleType={scheduleType}
              />
            )}

            {(() => {
              const cronString = getCronText(cronExpression);
              if (cronString.status) {
                return (
                  <p className="bg-card text-card-foreground border-input overflow-clip border p-3">
                    {cronString.value}{' '}
                    <span className="bg-accent text-accent-foreground p-1 px-2 font-mono">
                      cron({cronExpression})
                    </span>
                  </p>
                );
              }
              return (
                <p className="bg-destructive text-destructive-foreground border-destructive border p-3">
                  Invalid cron expression
                </p>
              );
            })()}
          </div>
        )}
      </div>
    </div>
  );
};
