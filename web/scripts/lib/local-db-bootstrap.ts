...
export type LocalDatabase = ReturnType<typeof createDatabase>['db'];

type UpsertCredentialUserInput = Readonly<{
  email: string;
  name: string;
  password: string;
  role: string | null;
...
export const readRequiredEnvString = (name: string): string => {
...
export const buildComponentParentTuple = (componentId: string, deviceId: string) => ({
...
export const upsertCredentialUser = async (
  ...
  input: UpsertCredentialUserInput,
) => {
...
export const upsertSysadmin = async (database: LocalDatabase) =>
  upsertCredentialUser(database, {
    email: process.env.LOCAL_SYSADMIN_EMAIL?.trim() || DEFAULT_SYSADMIN_EMAIL,
    name: process.env.LOCAL_SYSADMIN_NAME?.trim() || DEFAULT_SYSADMIN_NAME,
    password: process.env.LOCAL_SYSADMIN_PASSWORD?.trim() || DEFAULT_SYSADMIN_PASSWORD,
    role: 'admin',
  });
...
const ensureDeviceInstallations = async (
  ...
  if (missingInstallations.length > 0) {
    for (const chunk of chunkArray(missingInstallations, 500)) {
      await database.insert(deviceComponentInstallation).values(chunk);
    }
  }
...
export const syncDeviceComponentCatalog = async (database: LocalDatabase) => {
