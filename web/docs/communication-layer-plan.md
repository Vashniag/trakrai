...
3. `Next.js cloud UI`
   The UI does not talk to the device directly. For live view it connects to `live-gateway`. For normal control flows it should publish commands through a future generic cloud communication service.
...
1. Browser connects to `live-gateway`.
2. Browser selects `deviceId` and `cameraName`.
3. `live-gateway` publishes a `live-feed:start` command over MQTT.
...
7. Device and `live-gateway` exchange SDP/ICE via MQTT.
8. Browser receives the remote track from `live-gateway` signaling.

### Stop flow

9. Browser sends `stop`.
10. `live-gateway` publishes `live-feed:stop`.
11. Device tears down the session and sends final state.

## Why This Split Helps

- The broker remains reusable for future apps.
- WebRTC complexity stays isolated inside `live-gateway`.
...
2. Keep `live-gateway` as the dedicated live-view cloud service.
