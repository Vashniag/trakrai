...
// Pull one frame with timeout. Caller must free *data.
...
// Pipeline wraps a GStreamer pipeline whose final element is an appsink named "sink".
type Pipeline struct {
	h *C.PipelineHandle
}

// NewPipeline creates a pipeline from a gst_parse_launch description string.
func NewPipeline(desc string) (*Pipeline, error) {
	cd := C.CString(desc)
	defer C.free(unsafe.Pointer(cd))

	var cerr *C.char
	h := C.pipeline_create(cd, &cerr)
	if h == nil {
		msg := "unknown error"
		if cerr != nil {
			msg = C.GoString(cerr)
...
// PullFrame blocks up to timeoutNs for the next buffer from appsink.
...
// Stop tears down the pipeline and releases all resources.
