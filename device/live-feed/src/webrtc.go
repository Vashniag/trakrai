...
// SessionManager manages a single active WebRTC session at a time.
...
func NewSessionManager(cfg *Config, frameSrc *FrameSource, publisher MessagePublisher) *SessionManager {
...
// StartSession creates a new WebRTC peer connection and starts streaming.
func (sm *SessionManager) StartSession(cameraName string) {
	...
	// Wait for gathering in a goroutine, then send complete offer
	...
	// Start frame pump in a goroutine
	go sm.pumpFrames(ctx, cameraName, videoTrack)
}

// pumpFrames reads JPEG frames from Redis, encodes to H.264, and writes to WebRTC track.
func (sm *SessionManager) pumpFrames(ctx context.Context, cameraName string, track *webrtc.TrackLocalStaticSample) {
...
// SetRemoteAnswer sets the SDP answer received from the browser.
...
// StopSession stops the current WebRTC session.
