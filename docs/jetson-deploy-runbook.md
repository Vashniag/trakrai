...
End-to-end recipe for reflashing a Jetson-class device from a controller machine using published cloud packages. Companion to [`devtool.md`](/Users/hardikj/code/web-apps/trakrbi/trakrai/device/docs/devtool.md); that file explains the current CLI surface, while this runbook captures the tested operational procedure.
...
The bootstrap installer only replaces the entries listed in the staged bootstrap manifest, and `models` is not part of that managed set, so pre-staged weights survive a deploy.
...
4. The staged `bootstrap_device_runtime.py` helper runs as root on the device: stops the old units, creates `/home/hacklab/trakrai-device-runtime/`, installs binaries/configs/UI/wheels, writes the `trakrai-runtime-manager.service` unit, and waits for the runtime manager to materialize the per-service units.
...
`runtime-manager` and `cloud-comm` cannot be rolled forward by the managed-service update path the other services use — they *are* that path. `update_control_plane.py` is staged by the bootstrap helper into `<runtime-root>/scripts/` exactly to cover this case.
...
Default behavior: updates both `runtime-manager` and `cloud-comm` for `linux/arm64`, using the `package-versions.json` that was staged alongside the script by the last `python3 -m device.devtool deploy ssh ...` run.

To pull a fresher manifest than the one staged last deploy, either:

- `git pull` this repo on the controller, rerun `python3 -m device.devtool deploy ssh --config-dir … --start-mode core --keep-stage` (fast, only ~30 s since only the staged scripts/metadata change), then run the update script; or
